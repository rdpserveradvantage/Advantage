...
!empty($noOfAtm) && !empty($ATMID1) && !empty($city) && !empty($location) && !empty($LHO) && !empty($state)
        ...
        $sql = "insert into feasibilityCheck(ATMID1,ATMID2,ATMID3,AntennaRoutingdetail,EMLockPassword,EMlockAvailable,LHO,NoOfUps,PasswordReceived,Remarks,UPSAvailable,
            ...
            backroomKeyName,backroomKeyNumber,backroomKeyStatus,backroomNetworkRemark,city,earthing,earthingVltg,frequentPowerCut,frequentPowerCutFrom,frequentPowerCutRemark,
            frequentPowerCutTo,location,nearestShopDistance,nearestShopName,nearestShopNumber,noOfAtm,operator,powerFluctuationEN,powerFluctuationPE,powerFluctuationPN,
            powerSocketAvailability,routerAntenaPosition,signalStatus,state,status,created_at,powerSocketAvailabilityUPS,created_by,feasibilityDone,isVendor) values(
            ...
            '".$backroomKeyStatus."','".$backroomNetworkRemark."','".$city."','".$earthing."','".$earthingVltg."','".$frequentPowerCut."','".$frequentPowerCutFrom."',
            '".$frequentPowerCutRemark."','".$frequentPowerCutTo."','".$location."','".$nearestShopDistance."','".$nearestShopName."','".$nearestShopNumber."','".$noOfAtm."',
            '".$operator."','".$powerFluctuationEN."','".$powerFluctuationPE."','".$powerFluctuationPN."','".$powerSocketAvailability."','".$routerAntenaPosition."',
            '".$signalStatus."','".$state."',1,'".$created_at."','".$powerSocketAvailabilityUPS."','".$userid."','".$feasibilityDone."','1'
            )";
            ...
            where id= '".$insertid."'";
                ...
                if(mysqli_query($con,$update)){
                    ...
                    if($check_sql_result = mysqli_fetch_assoc($check_sql)){
                        $siteid = $check_sql_result['siteid'];
