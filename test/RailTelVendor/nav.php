<? 
// include('config.php');
if($_SESSION['VENDOR_username']){ 
    $id = $_SESSION['VENDOR_userid'];

     $user = "select * from vendorUsers where id=".$id;
...
$cpermission = "'" . implode ( "', '", $cpermission )."'";
   ...
   //  echo '<pre>';print_r($permission);echo '</pre>';
    foreach($permission as $key=>$val){
        ...
        $sub_menu_sql = mysqli_query($con,"select * from sub_menu where id='".$val."' and vendorStatus=1");
...
<nav class="pcoded-navbar">
                        ...
                        $menu_sql = mysqli_query($con,"select * from main_menu where id='".$menu_id."' and  vendorStatus=1");
